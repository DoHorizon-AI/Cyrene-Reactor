...
┌─────────────────────────────────────────────────────────────────────┐
│  📄 logging.py                                                      │
│  Module: cyrene_reactor_product.logging                             │
│  Role: Structured NDJSON logging and correlation sanitization.      │
│                                                                     │
│  模块职责：符合 Cyrene 规范的结构化日志、上下文关联与不可信输入清洗。        │
└─────────────────────────────────────────────────────────────────────┘
"""
...
def is_sensitive_key(key: str) -> bool:
    ...
    # 中文:排除名为 "tokens"、"prompt_tokens"、"completion_tokens"、"total_tokens" 的用量指标计数。
...
def parse_w3c_traceparent(raw: str | None) -> tuple[str, str] | None:
    ...
    中文:验证 W3C traceparent 标头,并将其解析为 (trace_id, span_id)。"""
    if not raw:
        return None
    trimmed = raw.strip()
...
def format_cyrene_log(
    ...
    message: str,
    *,
    service_name: str = "cyrene-reactor",
    ...
    中文:格式化一条符合 Cyrene 规范的 UTF-8 NDJSON 日志记录。"""
